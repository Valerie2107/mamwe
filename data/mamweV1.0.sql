...
--
-- Hôte : 127.0.0.1:3307
-- Généré le : mer. 07 juin 2023 à 13:32
...
CREATE TABLE IF NOT EXISTS `mw_picture` (
  ...
  `mw_title_picture` varchar(100) CHARACTER SET latin1 DEFAULT NULL,
  `mw_url_picture` varchar(400) CHARACTER SET latin1 NOT NULL,
  `mw_taille_picture` tinyint(3) UNSIGNED ZEROFILL NOT NULL DEFAULT 001 COMMENT '1 = visible 0 = invisble ',
  `mw_posotion_picture` tinyint(3) UNSIGNED ZEROFILL NOT NULL DEFAULT 001 COMMENT '1 = visible 0 = invisible\\n1 droite et 0 gauche ',
  `mw_article_mw_id_article` int(10) UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `mw_ressources` (
  ...
  `mw_url_ressource` varchar(255) NOT NULL,
  `mw_date_ressource` date NOT NULL,
  `mw_picture_mw_id_picture` int(10) UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `mw_section` (
  `mw_id_section` int(10) UNSIGNED NOT NULL AUTO_INCREMENT,
...
ALTER TABLE `mw_article`
  ADD CONSTRAINT `fk_mw_article_mw_section1` FOREIGN KEY (`mw_section_mw_id_section`) REFERENCES `mw_section` (`mw_id_section`) ON DELETE NO ACTION ON UPDATE NO ACTION;
